...
def evaluate_reverse_batch_rules(
    ...
    for k_batch, batch_data in tqdm.tqdm(
        ...
        val_0, ind_0 = torch.topk(torch.from_numpy(pre).cuda(), top, dim=-1)
        hr_n0, ndcg_n0 = batch_evaluate_return_array(ind_0[:,:re_top], target_item)
        hit_ratio_init += hr_n0.sum()
        ndcg_init += ndcg_n0.sum()
            
            
        pre_var = Variable(torch.empty_like(val_0, requires_grad=True)).cuda()
        with torch.no_grad():
             pre_var.copy_(torch.from_numpy(val_0.cpu().numpy()).data)
        ...
        for n_ in range(num):
            ...
            for p_num in range(p_num_):
                ...
                batch_lr[loss_.detach().cpu().numpy()<loss.detach().cpu().numpy()] = 1 #0.000 
                ...
                optimizer.step()
                # with torch.no_grad():
                #     pre_var[batch_lr[:,0]==1] = pre_var_ori[batch_lr[:,0]==1]
            ...
            with torch.no_grad():
                ...
                _, ind_rerank = torch.topk(pre_var, re_top, dim=-1)
                top_rerank = torch.gather(ind_0, -1, ind_rerank)  # ind[ind_rerank]

                hr, ndcg = batch_evaluate(top_rerank, target_item)
                hit_ratio_adj_ori[n_] += hr
                ndcg_adj_ori[n_] += ndcg

                # print("batch_lr shape:",batch_lr.shape)
                s_idx = torch.nonzero(batch_lr!=1)[:,0]
                s_idx2 = torch.nonzero(batch_lr==1)[:,0]
                
                user_num_n[n_] += s_idx.shape[0]
                

                hr, ndcg = batch_evaluate(top_rerank[s_idx], target_item[s_idx])
                hit_ratio_adj[n_] += hr + hr_n0[s_idx2].sum()
                ndcg_adj[n_] += ndcg + ndcg_n0[s_idx2].sum()
    ...
    print(
        ...
        'select recall ndcg:',
        hit_ratio_adj.max(), ndcg_adj.max()
        # "RI max:",
    ...
    print(
        ...
        ndcg_adj_ori.max(),
        "improve:",
        ndcg_adj_ori.max() - ndcg_init,
        "RI:",
        (ndcg_adj_ori.max() - ndcg_init)/ndcg_init
    )

    print('\n select hr:', hit_ratio_adj,
        'select ndcg:', ndcg_adj)
...
if __name__ == '__main__':
    print("amazon_cut_N_v2.....")
    # global dataset
    #Best config is: {'learning_rate': 0.01, 'learning_rate2': -1, 'learning_rate3': -1, 'num': 100, 'hid_pos': 3, 'top_': (50, 10), 'weight': 1, 'w': 0.5}
    #Best config is: {'learning_rate': 0.1, 'learning_rate2': -1, 'learning_rate3': -1, 'num': 100, 'hid_pos': 2, 'top_': (50, 10), 'weight': 0.1, 'w': 1}
    config = {
        'learning_rate':0.1, #tune.grid_search([100, 10, 1, 0.1, 0.01, 0.001]),
        ...
        'hid_pos':2, #tune.grid_search([100, 80, 20, 10]),
    ...
    # Best config is: {'lr': 0.001, 'l2': 1e-06, 'drop': 0.2, 'n_heads': 1}
    args.lr = 0.001
    args.l2_emb = 1e-6
    ...
    args.l2_emb = 1e-6
    args.dropout_rate = 0.2
    args.mode = '0215backward'
    save_backward = "amazon-SASRec"+args.mode+'lr' + str(args.lr)+"l2"+str(args.l2_emb)+"drop"+str(args.dropout_rate)+"h"+str(args.num_heads)+".pth" 
    ...
    # saved_forward_path = "/home/"
    # save_backward_path = "/home"
    # model_forward.load_state_dict(torch.load(saved_forward_path))
    # model_backward.load_state_dict(torch.load(save_backward_path))


    print("start prediction correcting process.....")
    print("validation...........")
    evaluate_reverse_batch_rules(
        model_forward,
        model_backward,
        train_csr,
        valid_sequences,
        ...
        None,
        top=config['top_'][0],
        re_top=config['top_'][1],
        valid_user_list=valid_data[:,0],
        batch_size=config_forward.batch_size,
        lr=config['learning_rate'],
        lr2=config['learning_rate2'],
        lr3=config['learning_rate3'],
        num=config['num'],
        T=config['weight'],
        weight=config['w']
    )
